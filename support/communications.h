/*
 *  Copyright (C) 2006-2022 Thomas d'Otreppe <tdotreppe@air.org>
...
#include <air/third-party/ieee80211.h>
#include "../support/ieee80211_compat.h"
...
static inline int get_ip_port(char * iface, char * ip, const int ip_size)
{
	...
	*ptr++ = 0;
	...
	port = (int) strtol(ptr, nullptr, 10);
...
static inline void read_sleep(int fd_in, unsigned long usec, read_sleep_cb cb)
{
		   ...
		   < (usec))
	{
		...
		if (select(fd_in + 1, &rfds, nullptr, nullptr, &tv3) < 0)
		{
			continue;
		}
							  ...
							  enum Send_Packet_Option option)
{
	...
	do
	{
		rc = wi_write(wi, nullptr, LINKTYPE_IEEE802_11, buf, (int) count, nullptr);
