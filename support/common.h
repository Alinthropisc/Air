...
[[nodiscard]] char * get_current_working_directory(void);

[[nodiscard]] int is_string_number(const char * str);

[[nodiscard]] int get_ram_size(void);

[[nodiscard]] char * getVersion(const char * progname,
...
[[nodiscard]] int get_nb_cpus(void);
...
{
	REQUIRE(mac_address != nullptr);

	char * mac_string = (char *) malloc(MAC_ADDRESS_STRING_LEN);
	ALLEGE(mac_string != nullptr);
...
[[nodiscard]] int hexCharToInt(unsigned char c);
...
[[nodiscard]] int getmac(const char * macAddress, const int strict, unsigned char * mac);
...
[[nodiscard]] int hexToInt(char s[], int len);
 ...
 *
 * Returns a pointer to the first occurrence of the character, or nullptr
