/*
 * WPA/WPA2/WPA3 crypto engine — PMK, PTK, MIC, PMKID derivation.
 *
...
AIR_EXPORT int ac_crypto_engine_supported_features(void)
{
...
AIR_EXPORT int ac_crypto_engine_simd_width(void)
{
#ifdef SIMD_COEF_32
	return SIMD_COEF_32;
#else
...
AIR_EXPORT int ac_crypto_engine_init(ac_crypto_engine_t * engine)
{
	...
	for (int i = 0; i < MAX_THREADS; ++i) engine->thread_data[i] = nullptr;

	return 0;
}

AIR_EXPORT void ac_crypto_engine_destroy(ac_crypto_engine_t * engine)
{
	REQUIRE(engine != nullptr);
#ifdef XDEBUG
...
AIR_EXPORT void ac_crypto_engine_set_essid(ac_crypto_engine_t * engine,
									   const uint8_t * essid)
{
	REQUIRE(engine != nullptr);
#ifdef XDEBUG
...
AIR_EXPORT int ac_crypto_engine_thread_init(ac_crypto_engine_t * engine,
										int threadid)
{
	REQUIRE(engine != nullptr);
#ifdef XDEBUG
...
AIR_EXPORT void ac_crypto_engine_thread_destroy(ac_crypto_engine_t * engine,
											int threadid)
{
	REQUIRE(engine != nullptr);
#ifdef XDEBUG
	...
	if (engine->thread_data[threadid] != nullptr)
	{
...
{
	return (uint8_t *) engine->thread_data[threadid]->ptk
	       + (DIGEST_SHA1_MAC_LEN * index);
}

AIR_EXPORT void ac_crypto_engine_calc_pke(ac_crypto_engine_t * engine,
...
AIR_EXPORT void ac_crypto_engine_calc_one_pmk(const uint8_t * key,
										  ...
										  uint8_t pmk[static PMK_LEN])
{
	...
	{
		fprintf(stderr, "air: fatal: PBKDF2-HMAC-SHA1 failed\n");
		abort();
	}
}

AIR_EXPORT void ac_crypto_engine_calc_pmk(
...
AIR_EXPORT void ac_crypto_engine_calc_ptk(ac_crypto_engine_t * engine,
...
AIR_EXPORT void ac_crypto_engine_calc_mic(ac_crypto_engine_t * engine,
									  ...
									  const int threadid)
{
	...
	{
		fprintf(stderr, "air: unsupported EAPOL key version %u\n", keyver);
		if (keyver == 0)
			fprintf(stderr, "air: hint: WPA3 PMKID path not yet wired here\n");
		/* TODO(rust): WPA3-SAE MIC verification belongs in the Rust async layer */
	}
}

AIR_EXPORT int ac_crypto_engine_wpa_crack(
	ac_crypto_engine_t * engine,
	const wpapsk_password key[MAX_KEYS_PER_CRYPT_SUPPORTED],
	const uint8_t eapol[256],
	const uint32_t eapol_size,
...
AIR_EXPORT void ac_crypto_engine_set_pmkid_salt(ac_crypto_engine_t * engine,
											const uint8_t bssid[6],
											const uint8_t stmac[6],
											int threadid)
{
...
AIR_EXPORT int ac_crypto_engine_wpa_pmkid_crack(
	ac_crypto_engine_t * engine,
	const wpapsk_password key[MAX_KEYS_PER_CRYPT_SUPPORTED],
	...
	const int threadid)
{
	...
	for (int j = 0; j < nparallel; ++j)
	{
		/* 8 ("PMK Name") + 6 (BSSID) + 6 (STA MAC) = 20 bytes salt */
		MAC_HMAC_SHA1(PMK_LEN, (const uint8_t *) &pmk[j], 20u, pke, l_pmkid);
