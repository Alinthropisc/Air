...
/* C23 does not need _POSIX_C_SOURCE; POSIX interfaces are available by default
 * when _DEFAULT_SOURCE is set (which clang -std=c23 implies on glibc).
 * Legacy define kept as a fallback for non-glibc targets. */
...
void * mem_alloc_tiny_func(size_t size, size_t align)
{
	 ...
	 * previous fix for this was returning nullptr but that lead to other
