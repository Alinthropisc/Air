 ...
 *
 *  Copyright (C) 2006-2018 Thomas d'Otreppe <tdotreppe@air.org>
...
#include "channel.h"
#include "../defs.h"
#include "../support/local_limits.h"
...
static int is_ndiswrapper(const char * iface, const char * path)
{
	...
	if ((pid = fork()) == 0)
	{
		...
		close(2);
		IGNORE_NZ(chdir("/"));
		execl(path, "iwpriv", iface, "ndis_reset", nullptr);
		exit(1);
	}
					  ...
					  struct rx_info * ri)
{
	...
	if (dev->arptype_in == ARPHRD_IEEE80211_FULL)
	{
		...
		if (ieee80211_radiotap_iterator_init(&iterator, rthdr, caplen, nullptr)
			< 0)
			return (0);
...
static int opensysfs(struct priv_linux * dev, char * iface, int fd)
{
	...
	if (iface == nullptr || strlen(iface) >= IFNAMSIZ)
	{
		return 1;
	}
...
static int set_monitor(struct priv_linux * dev, char * iface, int fd)
{
	...
	if (strcmp(iface, "prism0") == 0)
	{
		...
		if ((pid = fork()) == 0)
		{
			...
			close(2);
			IGNORE_NZ(chdir("/"));
			ALLEGE(dev->wl != nullptr);
			execl(dev->wl, "wl", "monitor", "1", nullptr);
			exit(1);
		}
		waitpid(pid, &status, 0);
	...
	if (!fork()) /* hostap */
	{
		...
		close(2);
		IGNORE_NZ(chdir("/"));
		execlp("iwpriv", "iwpriv", iface, "monitor_type", "1", nullptr);
		exit(1);
	}
	wait(nullptr);
	...
	if (!fork()) /* r8180 */
	{
		...
		close(2);
		IGNORE_NZ(chdir("/"));
		execlp("iwpriv", "iwpriv", iface, "prismhdr", "1", nullptr);
		exit(1);
	}
	wait(nullptr);
	...
	if (!fork()) /* prism54 */
	{
		...
		close(2);
		IGNORE_NZ(chdir("/"));
		execlp("iwpriv", "iwpriv", iface, "set_prismhdr", "1", nullptr);
		exit(1);
	}
	wait(nullptr);
...
static int do_linux_open(struct wif * wi, char * iface)
{
	...
	if (memcmp(iface, "eth", 3) == 0)
	{
		...
		if ((pid = fork()) == 0)
		{
			...
			close(2);
			IGNORE_NZ(chdir("/"));
			execlp("iwpriv", "iwpriv", iface, "get_port3", nullptr);
			exit(1);
		}
		...
		if ((pid = fork()) == 0)
		{
			...
			close(2);
			IGNORE_NZ(chdir("/"));
			execlp("iwpriv", "iwpriv", iface, "get_regdomain", nullptr);
			exit(1);
		}
	...
	if (dev->drivertype == DT_IPW2200)
	{
		...
		if ((acpi = fopen(r_file, "r")) == nullptr) goto close_out;
		memset(buf, 0, 128);
		...
		if (strncmp(buf, "-1", 2) == 0)
		{
			// repoen for writing
			fclose(acpi);
			if ((acpi = fopen(r_file, "w")) == nullptr) goto close_out;
			fputs("1", acpi);
			// reopen for reading
			fclose(acpi);
			if ((acpi = fopen(r_file, "r")) == nullptr) goto close_out;
	...
	if (memcmp(iface, "rtap", 4) == 0 && dev->main_if == nullptr)
	{
		memset(&ifr, 0, sizeof(ifr));
		strncpy(ifr.ifr_name, iface, sizeof(ifr.ifr_name) - 1);
		...
		while (net_ifaces != nullptr && (this_iface = readdir(net_ifaces)) != nullptr)
		{
			...
			if ((acpi = fopen(r_file, "r")) == nullptr) continue;
			...
			if (n == 0) // interface exists
			{
				if (strncmp(buf, iface, 5) == 0)
				{
					...
					if (dev->main_if == nullptr) continue;
			...
			else // need to create interface
			{
				if (strncmp(buf, "-1", 2) == 0)
				{
					// repoen for writing
					fclose(acpi);
					if ((acpi = fopen(r_file, "w")) == nullptr) continue;
					fputs("1", acpi);
					// reopen for reading
					fclose(acpi);
					if ((acpi = fopen(r_file, "r")) == nullptr) continue;
		...
		if (net_ifaces != nullptr) closedir(net_ifaces);
	}
...
EXPORT int get_battery_state(void)
{
	...
	if (linux_apm == 1)
	{
		char * battery_data = nullptr;

		if ((apm = fopen("/proc/apm", "r")) != nullptr)
		{
	...
	if (linux_acpi && !linux_apm)
	{
		...
		if (ac_adapters == nullptr) return 0;

		while ((this_adapter = readdir(ac_adapters)) != nullptr)
		{
			...
			snprintf(battery_state,
					 ...
					 this_adapter->d_name);
			if ((acpi = fopen(battery_state, "r")) == nullptr)
			{
				continue;
			}
			while (fgets(buf, 128, acpi))
			{
				if (strstr(buf, "on-line") != nullptr)
				{
					fclose(acpi);
		...
		while ((this_battery = readdir(batteries)) != nullptr)
		{
			...
			if (info != nullptr)
			{
				while (fgets(buf, sizeof(buf), info) != nullptr)
					if (sscanf(buf,
			...
			snprintf(battery_state,
					 ...
					 this_battery->d_name);
			if ((acpi = fopen(battery_state, "r")) == nullptr) continue;
			while (fgets(buf, 128, acpi))
			{
