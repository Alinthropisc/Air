...
get_80211(struct priv_nbsd * pn, int * plen, struct rx_info * ri)
{
	...
	REQUIRE(bpfh->bh_caplen == bpfh->bh_datalen); /* XXX */
...
static int do_nbsd_open(struct wif * wi, char * iface)
{
	...
	if (ioctl(fd, BIOCPROMISC, nullptr) < 0) goto close_bpf;
...
EXPORT int get_battery_state(void)
{
	...
	len = 1;
	value = 0;
	sysctlbyname("hw.acpi.acline", &value, &len, nullptr, 0);
	if (value == 0)
	{
		sysctlbyname("hw.acpi.battery.time", &value, &len, nullptr, 0);
		value = value * 60;
	}
	else
	{
