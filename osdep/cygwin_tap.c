...
{
	priv->tc_running = 2;
	if (pthread_create(&priv->tc_reader, nullptr, ti_reader, priv)) return -1;
...
ti_read_reg(struct tip_cygwin * priv, char * key, char * res, int len)
{
	...
	if (RegQueryValueEx(priv->tc_key, key, nullptr, &dt, (unsigned char *) res, &l)
		!= ERROR_SUCCESS)
		return -1;
...
static int ti_do_open_cygwin(struct tip_cygwin * priv)
{
	...
	{
		len = sizeof(name);
		if (RegEnumKeyEx(ak47, i, name, &len, nullptr, nullptr, nullptr, nullptr)
			!= ERROR_SUCCESS)
			break;
...
ti_is_us(struct tip_cygwin * priv, HDEVINFO * hdi, SP_DEVINFO_DATA * did)
{
	...
	return strstr(buf, "TAP-Win32") != nullptr;
}
...
static int ti_reset(struct tip_cygwin * priv)
{
	...
	hdi = SetupDiGetClassDevs(&GUID_DEVCLASS_NET, nullptr, nullptr, DIGCF_PRESENT);
...
static struct tif * ti_open_cygwin(char * iface)
{
	...
	if (pthread_mutex_init(&priv->tc_mtx, nullptr)) goto err;
