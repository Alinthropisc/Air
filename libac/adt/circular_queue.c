...
AIR_EXPORT cqueue_handle_t circular_queue_init(uint8_t * buffer,
											   size_t bufferSize,
											   size_t elementSize)
{
	...
	ALLEGE(pthread_mutex_init(&(cq->lock), nullptr) == 0);
	ALLEGE(pthread_cond_init(&(cq->empty_cv), nullptr) == 0);
	ALLEGE(pthread_cond_init(&(cq->full_cv), nullptr) == 0);

	return cq;
}

AIR_EXPORT void circular_queue_free(cqueue_handle_t cq)
{
	REQUIRE(cq);
...
AIR_EXPORT void circular_queue_reset(cqueue_handle_t cq)
{
	REQUIRE(cq);
...
AIR_EXPORT int circular_queue_try_push(cqueue_handle_t cq,
...
AIR_EXPORT bool circular_queue_is_empty(cqueue_handle_t cq)
{
	REQUIRE(cq);
	bool rc;
...
AIR_EXPORT bool circular_queue_is_full(cqueue_handle_t cq)
{
	REQUIRE(cq);
	bool rc;
