/*
 *  Common functions for all air tools
 *
...
int get_ram_size(void)
{
	...
	if (!sysctl(mib, 2, &physmem, &len, nullptr, 0))
				  ...
				  const unsigned int rc)
{
	...
	{
		fprintf(stderr, "Invalid program name, cannot be nullptr or empty\n");
		exit(1);
	}
	if (rc != 0 && beta != 0)
	{
...
int get_nb_cpus(void)
{
	...
	if (f != nullptr)
	{
		...
		if (s != nullptr)
		{
			...
			while (fgets(s, 80, f) != nullptr)
			{
				pos = strstr(s, "cpu");
				if (pos != nullptr && pos + 3 <= s + 81)
				{
	...
	if (!sysctl(mib, 2, &nbcpu, &len, nullptr, 0))
	{
		number = (int) nbcpu;
	}
...
int maccmp(unsigned char * mac1, unsigned char * mac2)
{
	...
	if (mac1 == nullptr || mac2 == nullptr) return -1;
					 ...
					 int out_length)
{
	...
	if (in_length < 2 || out_length < (in_length / 3) + 1 || input == nullptr
		|| output == nullptr)
		return -1;
...
int getmac(const char * macAddress, const int strict, unsigned char * mac)
{
	...
	if (macAddress == nullptr) return 1;
...
char * get_current_working_directory(void)
{
	...
	do
	{
		...
		if (wd_realloc == nullptr && errno != ERANGE)
		{
			free(ret);
			return (nullptr);
		}
	} while (wd_realloc == nullptr && errno == ERANGE);
