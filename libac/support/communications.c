...
int read_packet(struct wif * wi,
				...
				struct rx_info * ri)
{
	REQUIRE(buf != nullptr && count > 0);

	int rc;

	rc = wi_read(wi, nullptr, nullptr, buf, count, ri);
	if (rc == -1)
	{
		switch (errno)
		{
					...
					char * essid)
{
	...
	while (1)
	{
		...
		while (read_len < 22)
		{
			read_len = read_packet(wi, pkt_sniff, sizeof(pkt_sniff), nullptr);
		...
		if (bssid != nullptr && memcmp(bssid, pkt_sniff + 10, ETHER_ADDR_LEN) == 0
			&& *essid == '\0')
		{
			memset(essid, 0, 33);
		...
		if (bssid != nullptr && memcmp(bssid, NULL_MAC, ETHER_ADDR_LEN) == 0
...
/**
	if bssid != nullptr its looking for a beacon frame
*/
...
int capture_ask_packet(int * caplen, int just_grab)
{
	...
	while (1)
	{
		...
		if (opt.s_file == nullptr)
		{
			...
			if (select(dev.fd_in + 1, &rfds, nullptr, nullptr, &tv) < 0)
			{
			...
			*caplen = read_packet(_wi_in, h80211, sizeof(h80211), nullptr);
	...
	if (!just_grab)
	{
		...
		if ((f_cap_out = fopen(strbuf, "wb+")) == nullptr)
		{
			perror("fopen failed");
			return (EXIT_FAILURE);
		}
...
int dump_initialize_multi_format(char * prefix, int ivs_only)
{
	...
	if (opt.output_format_csv)
	{
		...
		if ((opt.f_txt = fopen(ofn, "wb+")) == nullptr)
		{
			perror("fopen failed");
	...
	if (opt.output_format_log_csv)
	{
		...
		if ((opt.f_logcsv = fopen(ofn, "wb+")) == nullptr)
		{
			perror("fopen failed");
			fprintf(stderr, "Could not create \"%s\".\n", ofn);
			free(ofn);
	...
	if (opt.output_format_kismet_csv)
	{
		...
		if ((opt.f_kis = fopen(ofn, "wb+")) == nullptr)
		{
			perror("fopen failed");
			fprintf(stderr, "Could not create \"%s\".\n", ofn);
			free(ofn);
	...
	if (opt.usegpsd)
	{
		...
		if ((opt.f_gps = fopen(ofn, "wb+")) == nullptr)
		{
			perror("fopen failed");
			fprintf(stderr, "Could not create \"%s\".\n", ofn);
			free(ofn);
	...
	if (opt.output_format_kismet_netxml)
	{
		...
		if ((opt.f_kis_xml = fopen(ofn, "wb+")) == nullptr)
		{
			perror("fopen failed");
			fprintf(stderr, "Could not create \"%s\".\n", ofn);
			free(ofn);
	...
	if (opt.output_format_pcap)
	{
		...
		if ((opt.f_cap = fopen(ofn, "wb+")) == nullptr)
		{
			perror("fopen failed");
			fprintf(stderr, "Could not create \"%s\".\n", ofn);
			free(ofn);
	...
	else if (ivs_only)
	{
		...
		if ((opt.f_ivs = fopen(ofn, "wb+")) == nullptr)
		{
			perror("fopen failed");
			fprintf(stderr, "Could not create \"%s\".\n", ofn);
			free(ofn);
...
int encrypt_data(uint8_t * data, size_t length)
{
	...
	if (opt.prga == nullptr && opt.crypt != CRYPT_WEP)
	{
