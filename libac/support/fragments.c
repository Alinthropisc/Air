			...
			int weplen)
{
	...
	if (smac == nullptr) return (-1);
	if (len <= 32 || len > 2000) return (-1);
	if (rFragment == nullptr) return (-1);
	...
	while (cur->next != nullptr)
	{
			...
			&& (wep == cur->wep))
		{
			...
			if (cur->fragment[frag] != nullptr) return (0);
...
int timeoutFrag(void)
{
	...
	while (cur->next != nullptr)
	{
		...
		if (timediff > FRAG_TIMEOUT)
		{
			...
			if (old->header != nullptr) free(old->header);
			for (i = 0; i < 16; i++)
				if (old->fragment[i] != nullptr) free(old->fragment[i]);
								...
								int weplen)
{
	...
	if (rFragment == nullptr) return (nullptr);
	if (smac == nullptr) return (nullptr);
	...
	while (cur->next != nullptr)
	{
		...
		if (memcmp(smac, old->source, 6) == 0 && old->sequence == sequence)
		{
			...
			if (old->fragnum == 0) return (nullptr);

			for (i = 0; i <= old->fragnum; i++)
			{
				if (old->fragment[i] == nullptr) return (nullptr);
				len += old->fragmentlen[i];
			}

			if (len > 2000) return (nullptr);
