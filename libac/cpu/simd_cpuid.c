...
struct _cpuinfo cpuinfo = {0, nullptr, nullptr, 0, 0, 0, 0, 0, 0, 0, 0.0, nullptr};
...
int cpuid_simdsize(int viewmax)
{
	...
	unsigned int max_level = __get_cpuid_max(0, nullptr);
...
static float cpuid_getcoretemp(void)
{
	...
	if (sysctlbyname("dev.cpu.0.temperature", &tempval, &len, nullptr, 0) == -1)
		return 0;

	cpuinfo.coretemp = (tempval - 2732) / 10.0f;
#elif __linux__
	if (cpuinfo.cputemppath != nullptr)
	{
		cpuinfo.coretemp
...
#define MAX_SENSOR_PATHS 16
	...
	while (cnt < (MAX_SENSOR_PATHS - 1) && (dp = readdir(dirp)) != nullptr)
	{
...
#elif __linux__
	FILE * cfd;
	char *line = nullptr, *token = nullptr;
	size_t linecap = 0;
	ssize_t linelen;
...
#elif __FreeBSD__
	if (sysctl(mib, 2, modelbuf, &len, nullptr, 0))
		snprintf(modelbuf, sizeof(modelbuf), "Unknown");

	pm = modelbuf;
#elif defined(__APPLE__) && defined(__aarch64__)
	if (sysctlbyname("machdep.cpu.brand_string", &modelbuf, &modelbuf_len, nullptr, 0))
...
int cpuid_getinfo(void)
{
	...
	if (cpuinfo.model != nullptr) printf("Model           = %s\n", cpuinfo.model);
	if (cpuinfo.flags != nullptr) printf("Features        = %s\n", cpuinfo.flags);
	...
	if (cpuinfo.flags != nullptr)
	{
		free(cpuinfo.flags);
		cpuinfo.flags = nullptr;
	}

	if (cpuinfo.model != nullptr)
	{
		free(cpuinfo.model);
		cpuinfo.model = nullptr;
	}

	if (cpuinfo.cputemppath != nullptr)
	{
		free(cpuinfo.cputemppath);
		cpuinfo.cputemppath = nullptr;
	}
